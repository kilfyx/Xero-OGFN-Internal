#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: AthenaHUDSituationalFeedback

#include "Basic.hpp"

#include "AthenaHUDSituationalFeedback_classes.hpp"
#include "AthenaHUDSituationalFeedback_parameters.hpp"


namespace SDK
{

// Function AthenaHUDSituationalFeedback.AthenaHUDSituationalFeedback_C.UpdateFeedbackStyles
// (Public, BlueprintCallable, BlueprintEvent)
// Parameters:
// class UDataTable*                       Style                                                  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UAthenaHUDSituationalFeedback_C::UpdateFeedbackStyles(class UDataTable* Style)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("AthenaHUDSituationalFeedback_C", "UpdateFeedbackStyles");

	Params::AthenaHUDSituationalFeedback_C_UpdateFeedbackStyles Parms{};

	Parms.Style = Style;

	UObject::ProcessEvent(Func, &Parms);
}


// Function AthenaHUDSituationalFeedback.AthenaHUDSituationalFeedback_C.ShowFeedback
// (BlueprintCallable, BlueprintEvent)

void UAthenaHUDSituationalFeedback_C::ShowFeedback()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("AthenaHUDSituationalFeedback_C", "ShowFeedback");

	UObject::ProcessEvent(Func, nullptr);
}


// Function AthenaHUDSituationalFeedback.AthenaHUDSituationalFeedback_C.HideFeedback
// (BlueprintCallable, BlueprintEvent)

void UAthenaHUDSituationalFeedback_C::HideFeedback()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("AthenaHUDSituationalFeedback_C", "HideFeedback");

	UObject::ProcessEvent(Func, nullptr);
}


// Function AthenaHUDSituationalFeedback.AthenaHUDSituationalFeedback_C.ExecuteUbergraph_AthenaHUDSituationalFeedback
// (Final, UbergraphFunction)
// Parameters:
// int32                                   EntryPoint                                             (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UAthenaHUDSituationalFeedback_C::ExecuteUbergraph_AthenaHUDSituationalFeedback(int32 EntryPoint)
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("AthenaHUDSituationalFeedback_C", "ExecuteUbergraph_AthenaHUDSituationalFeedback");

	Params::AthenaHUDSituationalFeedback_C_ExecuteUbergraph_AthenaHUDSituationalFeedback Parms{};

	Parms.EntryPoint = EntryPoint;

	UObject::ProcessEvent(Func, &Parms);
}


// Function AthenaHUDSituationalFeedback.AthenaHUDSituationalFeedback_C.Construct
// (BlueprintCosmetic, Event, Public, BlueprintEvent)

void UAthenaHUDSituationalFeedback_C::Construct()
{
	static class UFunction* Func = nullptr;

	if (Func == nullptr)
		Func = Class->GetFunction("AthenaHUDSituationalFeedback_C", "Construct");

	UObject::ProcessEvent(Func, nullptr);
}

}

