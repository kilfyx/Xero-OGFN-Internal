...
void PostRender(SDK::UGameViewportClient* Viewport, SDK::UCanvas* Canvas)
{
	...
	for (int i = 0; i < Players.Num(); i++)
	{
		...
		if (jew::settings.bGun)
		{
			...
			Canvas->K2_DrawText(UE->DrawingFont, WeaponName, FVector2D(vRootScreen.X, vRootScreen.Y + 10), FVector2D(1.0, 1.0), RenderColor, 0, FLinearColor(0.0f, 0.f, 0.f, 0.f), FVector2D(0, 0), true, false, true, FLinearColor(0.0f, 0.0f, 0.0f, 0.6f));
		}
		...
		if (jew::settings.bBox)
		{
			if (IsVisible)
			{
				ZeroGUI::Draw_Line(FVector2D(BottomLeftX, BottomLeftY), FVector2D(BottomRightX, BottomRightY), 1.f, iColor);
				ZeroGUI::Draw_Line(FVector2D(BottomRightX, BottomRightY), FVector2D(TopRightX, TopRightY), 1.f, iColor);
				ZeroGUI::Draw_Line(FVector2D(TopRightX, TopRightY), FVector2D(TopLeftX, TopLeftY), 1.f, iColor);
				ZeroGUI::Draw_Line(FVector2D(TopLeftX, TopLeftY), FVector2D(BottomLeftX, BottomLeftY), 1.f, iColor);
			}
			else
			{
				ZeroGUI::Draw_Line(FVector2D(BottomLeftX, BottomLeftY), FVector2D(BottomRightX, BottomRightY), 1.f, FLinearColorPalette::Gray);
				ZeroGUI::Draw_Line(FVector2D(BottomRightX, BottomRightY), FVector2D(TopRightX, TopRightY), 1.f, FLinearColorPalette::Gray);
				ZeroGUI::Draw_Line(FVector2D(TopRightX, TopRightY), FVector2D(TopLeftX, TopLeftY), 1.f, FLinearColorPalette::Gray);
				ZeroGUI::Draw_Line(FVector2D(TopLeftX, TopLeftY), FVector2D(BottomLeftX, BottomLeftY), 1.f, FLinearColorPalette::Gray);
			}
		}
	...
	if (jew::settings.bAimbot && ClosestPlayer)
	{
		if (auto CurrentWeapon = AcknowledgedPawn->CurrentWeapon)
		{
			...
			if (CurrentWeapon->IsProjectileWeapon() && jew::settings.bPrediction)
			{
				HeadBone = Utils::Predict(CameraLocation, HeadBone, ClosestPlayer->GetVelocity(), CurrentWeapon);
			}

			if (GetAsyncKeyState(jew::settings.iAimbotKey))
			{
				AimAtTargetMouse(CameraLocation, CameraRotation, HeadBone, jew::settings.fSmooth);
			}
		}
	}
}
...
void SetupClasses()
{
	...
	UE->RightShift = SDK::FKey{ SDK::FName{UE->KismetStringLibrary->Conv_StringToName(L"Right Shift") } };
