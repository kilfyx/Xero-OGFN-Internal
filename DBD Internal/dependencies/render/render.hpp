...
namespace ZeroGUI
{
	namespace Colors
	{
		...
		inline FLinearColor Checkbox_Enabled = Button_Active;

		inline FLinearColor Combobox_Idle = Button_Idle;
	...
	inline void Draw_Cursor(bool toogle)
	{
		if (toogle)
		{
			...
			UE->CachedCanvas->K2_DrawLine(FVector2D{ cursorPos.X, cursorPos.Y }, FVector2D{ cursorPos.X + 35, cursorPos.Y + 10 }, 1, FLinearColor{ 0.30f, 0.30f, 0.80f, 1.0f });
			...
			while (y != 30) //20 steps
			{
				...
				UE->CachedCanvas->K2_DrawLine(FVector2D{ cursorPos.X, cursorPos.Y }, FVector2D{ cursorPos.X + x, cursorPos.Y + y }, 1, FLinearColor{ 0.30f, 0.30f, 0.80f, 1.0f });
			}

			UE->CachedCanvas->K2_DrawLine(FVector2D{ cursorPos.X, cursorPos.Y }, FVector2D{ cursorPos.X + 15, cursorPos.Y + 30 }, 1, FLinearColor{ 0.30f, 0.30f, 0.80f, 1.0f });
			UE->CachedCanvas->K2_DrawLine(FVector2D{ cursorPos.X + 35, cursorPos.Y + 10 }, FVector2D{ cursorPos.X + 15, cursorPos.Y + 30 }, 1, FLinearColor{ 0.30f, 0.30f, 0.80f, 1.0f });
		}
	}
	...
	inline void TextLeft(const char* name, FVector2D pos, FLinearColor color, bool outline)
	{
		...
		UE->CachedCanvas->K2_DrawText(UE->Font, s2wc(name), pos, FVector2D{ 0.97f, 0.97f }, color, false, Colors::Text_Shadow, FVector2D{ pos.X + 1, pos.Y + 1 }, false, true, true, Colors::Text_Outline);
	}
	inline void TextCenter(const char* name, FVector2D pos, FLinearColor color, bool outline)
	{
		int length = strlen(name) + 1;
		UE->CachedCanvas->K2_DrawText(UE->Font, s2wc(name), pos, FVector2D{ 0.97f, 0.97f }, color, false, Colors::Text_Shadow, FVector2D{ pos.X + 1, pos.Y + 1 }, true, true, true, Colors::Text_Outline);
	}
	...
	inline void Draw_Line(FVector2D from, FVector2D to, int thickness, FLinearColor color)
	{
		UE->CachedCanvas->K2_DrawLine(FVector2D{ from.X, from.Y }, FVector2D{ to.X, to.Y }, thickness, color);
	}
	inline void drawFilledRect(FVector2D initial_pos, float w, float h, FLinearColor color)
	{
		for (float i = 0.0f; i < h; i += 1.0f)
			UE->CachedCanvas->K2_DrawLine(FVector2D{ initial_pos.X, initial_pos.Y + i }, FVector2D{ initial_pos.X + w, initial_pos.Y + i }, 1.0f, color);
	}

	inline void filled_box(FVector2D position, FVector2D size, FLinearColor render_color) {
		for (int i = 0; i < size.Y; i++) {
			UE->CachedCanvas->K2_DrawLine(FVector2D(position.X, position.Y + i), FVector2D(position.X + size.X, position.Y + i), 1.f, render_color);
		}
	}
	inline void circle(FVector2D position, int radius, int segments, FLinearColor color) {
		float PI = 3.1415927f;
		...
		for (float a = 0; a < PI * 2.0; a += Step) {
			float X1 = radius * UE->KismetMathLibrary->Cos(a) + position.X;
			float Y1 = radius * UE->KismetMathLibrary->Sin(a) + position.Y;
			float X2 = radius * UE->KismetMathLibrary->Cos(a + Step) + position.X;
			float Y2 = radius * UE->KismetMathLibrary->Sin(a + Step) + position.Y;
			V[Count].X = X1;
			V[Count].Y = Y1;
			V[Count + 1].X = X2;
			V[Count + 1].Y = Y2;
	...
	inline void Checkbox(const char* name, bool* value)
	{
		elements_count++;
	...
	inline void Hotkey(const char* name, FVector2D size, int* key)
	{
		elements_count++;
